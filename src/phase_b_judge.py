...
# Console mặc định trên Windows (cp1252) không encode được tiếng Việt có dấu → crash
# khi print(). Ép stdout/stderr sang UTF-8 nếu có thể.
...
1. Độ chính xác: có đúng với chính sách/dữ kiện không? (quan trọng nhất)
2. Đầy đủ: có trả lời TRỌN VẸN phần TRỌNG TÂM câu hỏi hỏi tới không?
   (không yêu cầu liệt kê thêm chi tiết nằm ngoài phạm vi câu hỏi)
3. Súc tích: không lan man, không thừa thông tin không liên quan.

Lưu ý: nếu CẢ HAI answer đều chính xác và đã trả lời đủ trọng tâm câu hỏi,
hãy chọn "tie" — KHÔNG chọn answer dài/chi tiết hơn chỉ vì nó có thêm thông
tin phụ mà câu hỏi không yêu cầu. Chỉ chọn một answer thắng rõ ràng khi
answer còn lại SAI sự thật hoặc THIẾU thông tin cốt lõi cần để trả lời đúng câu hỏi.
...
'''
    try:
        ...
        result.setdefault("scores", {"A": 0.0, "B": 0.0})
        return result
...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    scores_a = pass1.get("scores", {}) or {}
    scores_b_raw = pass2_raw.get("scores", {}) or {}
    ...
    return JudgeResult(
        ...
        scores_pass1=scores_a,
        scores_pass2={"A": scores_b_raw.get("B", 0.0), "B": scores_b_raw.get("A", 0.0)},
    )
...
def cohen_kappa(judge_labels: list[int], human_labels: list[int]) -> float:
    ...
    p_o = sum(j == h for j, h in zip(judge_labels, human_labels)) / n
    p_judge_1 = judge_labels.count(1) / n
    p_human_1 = human_labels.count(1) / n
    p_e = p_judge_1 * p_human_1 + (1 - p_judge_1) * (1 - p_human_1)
...
def save_phase_b_report(judge_items: list[dict], kappa: float, bias: dict,
                         path: str = "reports/judge_results.json") -> None:
    """Save Phase B report to JSON."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
    report = {
        "num_judged": len(judge_items),
...
if __name__ == "__main__":
    ...
    # Judge mỗi model_answer (A) đấu với ground_truth (B) qua swap-and-average.
    # final_winner == "A" hoặc "tie" → judge cho là answer OK (label 1)
    # final_winner == "B" (ground truth thắng rõ ràng) → judge cho là answer sai (label 0)
    with open(HUMAN_LABELS_PATH, encoding="utf-8") as f:
        human_data = json.load(f)
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        test_set = {item["id"]: item for item in json.load(f)}
    print(f"\nHuman labels loaded: {len(human_data)} questions")
    ...
    for item in human_data:
        ...
        jr = swap_and_average(item["question"], item["model_answer"], ground_truth)
        judge_label = 1 if jr.final_winner in ("A", "tie") else 0
        ...
        judge_items.append({
            ...
            "reasoning_pass1": jr.reasoning_pass1,
        })
        agree = "✓" if judge_label == item["human_label"] else "✗"
        print(f"  [{qid:>2}] judge={judge_label} human={item['human_label']} {agree}  {item['question'][:45]}")
    ...
    save_phase_b_report(judge_items, kappa, bias)
