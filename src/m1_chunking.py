...
def _get_semantic_model():
    """Lazy-load & cache the sentence embedding model (avoid reloading per call)."""
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    # 1) Group paragraphs into parent chunks (≤ parent_size chars each).
    parents: list[Chunk] = []
    current = ""
    ...
    # 2) Split each parent into children (≤ child_size chars), preferring sentence boundaries.
    def _split_by_size(ptext: str, size: int) -> list[str]:
        sentences = re.split(r'(?<=[.!?])\s+', ptext)
        pieces, curr = [], ""
        for s in sentences:
            if len(curr) + len(s) > size and curr:
                pieces.append(curr.strip())
                curr = ""
            curr += s + " "
        if curr.strip():
            pieces.append(curr.strip())
        # Hard-split any piece still longer than size (e.g. one giant sentence).
        result = []
        for piece in pieces:
            while len(piece) > size:
                result.append(piece[:size].strip())
                piece = piece[size:]
            if piece.strip():
    ...
    for parent in parents:
        ...
        for j, child_text in enumerate(_split_by_size(parent.text, child_size)):
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    def _flush():
        ...
        if content:
            ...
            chunks.append(Chunk(text=full_text,
