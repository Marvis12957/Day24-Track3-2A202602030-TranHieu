...
# Chỉ quan tâm các loại PII thực sự liên quan (CCCD/CMND, SĐT VN, email).
# Corpus HR ở đây là tiếng Việt nhưng Presidio's NLP engine (spaCy en_core_web_lg)
# chỉ hỗ trợ tiếng Anh — nếu để analyzer chạy full recognizer set (PERSON, LOCATION,
# DATE_TIME dựa trên spaCy NER) thì gần như MỌI câu tiếng Việt đều bị false-positive
# (từ viết hoa/không dấu bị NER tiếng Anh nhận nhầm là PERSON/LOCATION).
# → Giới hạn entities= để chỉ chạy các recognizer liên quan tới bài toán.
PII_TARGET_ENTITIES = ["VN_CCCD", "VN_PHONE", "EMAIL_ADDRESS", "PHONE_NUMBER"]
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # (colang flows) hoặc bằng self-check-input (câu trả lời từ chối chung).
    refuse_keywords = ["xin lỗi", "không thể", "không được phép", "i cannot", "i'm sorry",
                        "i can't", "i am unable", "i'm unable"]
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    if isinstance(response, dict):
        response = response.get("content", "")
    refuse_keywords = ["xin lỗi", "không thể cung cấp", "không thể", "i cannot", "i can't"]
...
def save_phase_c_report(pii_demo: dict, adv_results: list[dict], latency: dict,
...
if __name__ == "__main__":
    ...
    # Shared engines — dựng 1 lần, tái dùng cho adversarial suite + latency (đúng thực tế production)
    rails = setup_nemo_rails()
    analyzer, anonymizer = setup_presidio()
    ...
    # Task 12: P95 latency — đo trên toàn bộ 20 adversarial inputs
    sample_inputs = [item["input"] for item in adversarial_set]
    latency = measure_p95_latency(sample_inputs, n_runs=len(sample_inputs),
    ...
    save_phase_c_report(result, results, latency)
